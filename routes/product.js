...
class QueryFeatures {
    ...
    filtering() {
        ...
        if (queryObject['category']) queryObject['category']['in'] = queryObject['category']['in'].split(',');
        if (queryObject['size']) queryObject['size']['in'] = queryObject['size']['in'].split(',');
        if (queryObject['sleeve']) queryObject['sleeve']['in'] = queryObject['sleeve']['in'].split(',');
        if (queryObject['color']) queryObject['color']['in'] = queryObject['color']['in'].split(',');
        if (queryObject['title']) {
            queryObject['title']['$options'] = 'i';
            queryObject['title']['regex'] = queryObject['title']['regex'].replace(/[.\"\'\`!*-+={}?^$()|[\]\\]/g, '');
        };
        let queryStr = JSON.stringify(queryObject);
        queryStr = queryStr.replace(/\b(gte|gt|lt|lte|in|regex)\b/g, match => '$' + match);
    ...
    sorting() { 
        if (this.requestQuery.sort){
            if (this.requestQuery.sort === 'createdAt:1'){
                ...
                this.databaseQuery = this.databaseQuery.sort({[sortField]: sortValue,'createdAt':-1});        
            };
        } else {
            this.databaseQuery = this.databaseQuery.sort({createdAt: -1, views:-1});
        };
        return this;
    };
...
router.delete('/:id',auth, async(req, res)=>{
    ...
    try {
        ...
        images.forEach(async img=>{
            await cloudinary.v2.uploader.destroy(img.public_id, async(err, result) =>{
                if (err) throw err;
            });
        });
